...
/**
 * Higher Order Component that wraps admin pages with protection
 * 
 * This HOC:
 * 1. Verifies client-side admin status from AuthContext
 * 2. Sets up CSRF protection
 * 3. Verifies server-side admin status
 * 4. Shows appropriate access denied pages
 * 5. Renders the wrapped component only when all checks pass
 */
export function withAdminProtection<P extends object>(
  ...
  return function AdminProtectedComponent(props: P) {
    ...
    useEffect(() => {
      ...
      // Skip server verification if not required
    ...
    }, [user, isAdmin, requireVerification]);
    ...
    if (loading || (!serverVerified && requireVerification)) {
      return (
        <div className="min-h-screen flex flex-col">
          <Navbar />
          <main className="flex-grow flex items-center justify-center">
